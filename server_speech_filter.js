...
/* ==========================================================================
   server_speech_filter.js — XXL Module
   ========================================================================= */
...
function normalizeAffirmations(text='') {
  const t = String(text||'').toLowerCase();
  if (/(^|\b)(y(es|ep|eah|up)|sure|ok(ay)?|go ahead|please do|affirmative|uh huh)($|\b)/.test(t)) return 'yes';
  if (/(^|\b)(no(pe)?|nah|negative|not now|maybe later|uh uh)($|\b)/.test(t)) return 'no';
  return 'unknown';
}
...
function expandStateAbbreviations(text='') {
  return String(text||'').replace(/([A-Z]{2})/g, (m, code) => STATE_MAP[code] || m);
}

// Stub ASR
function asrStub(audioMeta) {
  return { ok: true, transcript: '[asr:stub] hello there (sanitized)', confidence: 0.75, engine: 'stub-asr' };
}
// Stub NLP
function nlpStub(text) {
  const t = normalizeAffirmations(text);
  return { ok: true, intent: t==='yes'?'AFFIRM':t==='no'?'NEGATE':'UNKNOWN', entities: [], sentiment: 0.1 };
}

function attach(app, opts={}) {
  const base = opts.base || '/speech-filter';
  const router = express.Router();

  router.get('/health', (req, res) => res.json({ status: 'UP', service: 'speech_filter', version: '1.0.0' }));

  router.post('/sanitize', (req, res) => {
    const { text } = req.body || {};
    const clean = expandStateAbbreviations(sanitize(text||''));
    res.json({ ok: true, input: text||'', output: clean });
  });

  router.post('/asr', (req, res) => {
    const { audio } = req.body || {};
    const out = asrStub(audio);
    out.transcript = expandStateAbbreviations(sanitize(out.transcript));
    res.json(out);
  });
  ...
  router.post('/intent', (req, res) => {
    const { text } = req.body || {};
    const clean = expandStateAbbreviations(sanitize(text||''));
    const out = nlpStub(clean);
    res.json({ ok: true, input: text||'', sanitized: clean, nlp: out });
  });

  app.use(base, router);
}

module.exports = { attach, sanitize, normalizeAffirmations, expandStateAbbreviations };

/*
==============================================================================
Speech Filter Runbooks & SOP Index
==============================================================================
...
/*
==============================================================================
Speech Filter Deep Dive: Pipelines & Failure Modes
==============================================================================
* Runbook: Startup & Shutdown — Step 1: See SOP #100 for exact command sequence and expected outcomes.
* Runbook: Incident Response (P0/P1/P2) — Step 2: See SOP #101 for exact command sequence and expected outcomes.
* Runbook: Health Checks & Dashboards — Step 3: See SOP #102 for exact command sequence and expected outcomes.
